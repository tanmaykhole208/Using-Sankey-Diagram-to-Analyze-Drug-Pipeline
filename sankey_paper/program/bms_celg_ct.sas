...
proc import datafile="%sysfunc(pathname(fmydoc,f))/bms_ct_20191122.csv"
     out=bms_ct
     dbms=csv
     replace;
     getnames=yes;
...
proc import datafile="%sysfunc(pathname(fmydoc,f))/celg_ct_20191122.csv"
     out=cc_ct
     dbms=csv
     replace;
     getnames=yes;
...
proc freq data=bms_ct_;
	...
	table SPNSR*Status*Interventions*Conditions*Phases / list missing out=bms_wt;
...
proc sql noprint ;
	select name into: cc_int separated by " ne ' ' and spnsrn=1 then delete; if "
	from bms_ct_cont0
...
%cond_int_ph_wt;
	...
	/*OLD: https://jsfiddle.net/tanmaykhole208/z4e9xdwf/19*/
	/*NEW: https://jsfiddle.net/tanmaykhole208/z19vcjuq/85/*/

	proc sql;
		*create table all_spnsr_wt as
			select distinct a.*, "            { 'source': '"||strip(a.spnsr_comb)||"', 'target': '"||strip(a.spnsr)||"', 'value': "||strip(put(spnsr_ct, 5.0))||"}," as final length=1000
			from 
			(select spnsr_comb, spnsr, sum(count) as spnsr_ct
			 from all_cond_int_ph
			 group by spnsr) a
			;* 
			left join 
			(select spnsr, sum(count) as tot_ct
			 from all_cond_int_ph) b
			 on a.spnsr=b.spnsr
			;


		*create table all_spnsr_wt as
			select distinct spnsr, sum(count) as spnsr_ct, "            [ '"||strip(spnsr_comb)||"', '"||strip(spnsr)||"', "||strip(put(sum(count), 5.0))||"]," as final length=1000
			from all_cond_int_ph
			group by spnsr
			;
		...
		create table all_cond_wt as
			select distinct a.*, 
					case 
						when a.spnsr="BMS" then "            	{ 'source': '"||strip(a.spnsr)||"', 'target': '"||strip(a.cond1)||"', 'value': "||strip(put(cond_ct, 5.0))||"},"
						else                    "            	{ 'source': '"||strip(a.spnsr)||"', 'target': '"||strip(a.cond1)||" ', 'value': "||strip(put(cond_ct, 5.0))||"},"				  
					end as final length=1000
			from 
			(select distinct cond, cond1, spnsr, sum(count) as cond_ct
			 from all_cond_int_ph
			 group by spnsr, cond) a
			 ;*
			left join 
			(select spnsr, sum(count) as spnsr_ct
			 from all_cond_int_ph
			 group by spnsr) b
			 on a.spnsr=b.spnsr
			;
		...
		create table all_cond_int_wt as
			select distinct a.*, 
					case 
						when a.spnsr="BMS" then "            		{ 'source': '"||strip(a.cond1)||"', 'target': '"||strip(a.int)||"', 'value': "||strip(put(cond_int_ct, 5.0))||"},"
						else                    "            		{ 'source': '"||strip(a.cond1)||" ', 'target': '"||strip(a.int)||"', 'value': "||strip(put(cond_int_ct, 5.0))||"},"				  
					end as final length=1000
			from 
			(select distinct spnsr, cond, cond1, int, sum(count) as cond_int_ct
			 from all_cond_int_ph
			 group by spnsr, cond, int) a
			 ;*
			left join 
			(select distinct cond, cond1, spnsr, sum(count) as cond_ct
			 from all_cond_int_ph
			 group by spnsr, cond) b
			 on a.spnsr=b.spnsr and a.cond=b.cond
			;

		create table all_int_ph_wt as
			select distinct a.*,  
					case 
						when a.spnsr="BMS" then "            			{ 'source': '"||strip(a.int)||"', 'target': '"||strip(a.phases)||"', 'value': "||strip(put(int_ph_ct, 5.0))||"},"
						else 				    "            			{ 'source': '"||strip(a.int)||"', 'target': '"||strip(a.phases)||" ', 'value': "||strip(put(int_ph_ct, 5.0))||"},"
					end as final length=1000
			from 
			(select distinct spnsr, int, phases, sum(count) as int_ph_ct
			 from all_cond_int_ph
			 group by spnsr, int, phases) a
			 ;*
			left join
			(select distinct spnsr, cond, cond1, int, sum(count) as cond_int_ct
			 from all_cond_int_ph
			 group by spnsr, cond, int) b
			 on a.spnsr=b.spnsr and a.int=b.int
			;
	quit;
	...
	proc sql noprint;
			select final into: sankeydata2 separated by " "
			from final_wt
			;
		quit;
...
%sankey2html(in_data=%nrbquote(&sankeydata2.), outfl=%sysfunc(pathname(outg,f))/sank2.html, width=2000, height=800, flow_num=);


/***********Separate Conditions and Interventions for simplicity************/

	data all_cond_int_ph;
		set all_cond_int_ph;
			if spnsr="CELGENE" then int=strip(int)||" * ";
	run;

/**************ST******************/
	/*OLD: https://jsfiddle.net/tanmaykhole208/d052bme6/9*/
	/*NEW: https://jsfiddle.net/tanmaykhole208/cs1qbuLx/22/*/

	proc sql;
		create table all_cond_wt_st as
			select distinct cond, spnsr_comb, sum(count) as cond_ct, 
					"            	{ 'source': '"||strip(spnsr_comb)||"', 'target': '"||strip(cond1)||"', 'value': "||strip(put(sum(count), 5.0))||"}," as final length=1000
			from all_cond_int_ph
			where cond="ST"
			;

		create table all_cond_int_wt_st as
			select distinct spnsr_comb, cond, int, sum(count) as cond_int_ct, 
					"            	{ 'source': '"||strip(cond1)||"', 'target': '"||strip(int)||"', 'value': "||strip(put(sum(count), 5.0))||"}," as final length=1000
			from all_cond_int_ph
			where cond="ST"
			group by spnsr_comb, cond, int
			;
		create table all_int_ph_wt_st as
			select distinct spnsr_comb, int, phases, sum(count) as int_ph_ct, 
					"            	{ 'source': '"||strip(int)||"', 'target': '"||strip(phases)||"', 'value': "||strip(put(sum(count), 5.0))||"}," as final length=1000
			from all_cond_int_ph
			where cond="ST"
			group by spnsr_comb, int, phases
			;
	quit;


	data final_wt_st;
		set all_cond_wt_st all_cond_int_wt_st all_int_ph_wt_st;
		keep final;
	run;


/**************LL MM BT ******************/
	/*OLD: https://jsfiddle.net/tanmaykhole208/nagu8h3d/6*/
	/*NEW: https://jsfiddle.net/tanmaykhole208/bdwtajfq/10/*/

	proc sql;
		create table all_cond_wt_ll as
			select distinct cond, spnsr_comb, sum(count) as cond_ct, 
					"            	{ 'source': '"||strip(spnsr_comb)||"', 'target': '"||strip(cond1)||"', 'value': "||strip(put(sum(count), 5.0))||"}," as final length=1000
			from all_cond_int_ph
			where cond in ("LL" "MM" "B_T")
			group by spnsr_comb, cond
			;

		create table all_cond_int_wt_ll as
			select distinct spnsr_comb, cond, int, sum(count) as cond_int_ct, 
					"            	{ 'source': '"||strip(cond1)||"', 'target': '"||strip(int)||"', 'value': "||strip(put(sum(count), 5.0))||"}," as final length=1000
			from all_cond_int_ph
			where cond in ("LL" "MM" "B_T")
			group by spnsr_comb, cond, int
			;
		create table all_int_ph_wt_ll as
			select distinct spnsr_comb, int, phases, sum(count) as int_ph_ct, 
					"            	{ 'source': '"||strip(int)||"', 'target': '"||strip(phases)||"', 'value': "||strip(put(sum(count), 5.0))||"}," as final length=1000
			from all_cond_int_ph
			where cond in ("LL" "MM" "B_T")
	...
	data final_wt_ll;
		set all_cond_wt_ll all_cond_int_wt_ll all_int_ph_wt_ll;
		keep final;
	run;

/**************II CV******************/
	/*OLD: https://jsfiddle.net/tanmaykhole208/ns6zqf2y/3*/
	/*NEW: https://jsfiddle.net/tanmaykhole208/1w69Lgvr/11/*/

	proc sql;
		create table all_cond_wt_ii as
			select distinct cond, spnsr_comb, sum(count) as cond_ct, 
					"            	{ 'source': '"||strip(spnsr_comb)||"', 'target': '"||strip(cond1)||"', 'value': "||strip(put(sum(count), 5.0))||"}," as final length=1000
			from all_cond_int_ph
			where cond in ("II" "CV")
		...
		create table all_cond_int_wt_ii as
			select distinct spnsr_comb, cond, int, sum(count) as cond_int_ct, 
					"            	{ 'source': '"||strip(cond1)||"', 'target': '"||strip(int)||"', 'value': "||strip(put(sum(count), 5.0))||"}," as final length=1000
			from all_cond_int_ph
			where cond in ("II" "CV")
			group by spnsr_comb, cond, int
			;
		create table all_int_ph_wt_ii as
			select distinct spnsr_comb, int, phases, sum(count) as int_ph_ct, 
					"            	{ 'source': '"||strip(int)||"', 'target': '"||strip(phases)||"', 'value': "||strip(put(sum(count), 5.0))||"}," as final length=1000
			from all_cond_int_ph
			where cond in ("II" "CV")
	...
	data final_wt_ii;
		set all_cond_wt_ii all_cond_int_wt_ii all_int_ph_wt_ii;
		keep final;
	run;
