...
data AMG_ct_;
	set AMG_ct(in=a) cc_ct(in=b drop= START_DATE PRIMARY_COMPLETION_DATE COMPLETION_DATE RESULTS_FIRST_POSTED);

	  if substr(strip(upcase(sponsor_collaborators)), 1, 5) = "AMGEN" or 
			(substr(strip(upcase(sponsor_collaborators)), 1, 7) = "CELGENE" and 
				((index(upcase(Interventions), "OTEZLA") > 0 or index(upcase(Interventions), "APREMILAST") > 0) or
...
proc freq data=AMG_ct_ noprint ;
	table Status / list missing out= AMG_status;
	table Study_Results / list missing ;
	table Conditions / list missing out=AMG_cond;
	table Interventions / list missing out=AMG_int;
	table Outcome_Measures / list missing out=AMG_out_meas;
	table Phases / list missing out=AMG_phases;
	table SPNSR / list missing out=spnsr_wt;
	table SPNSR*Status*Interventions*Conditions*Phases / list missing out=AMG_wt;
...
proc contents data=AMG_ct_da out=AMG_ct_cont noprint ;
run;


proc sql noprint ;
	select name into: AMG_cc_int separated by " = ' ' and "
	from AMG_ct_cont
	where substr(name,1,3) = "AMG" 
	;
quit;
...
proc sql noprint ;
	select distinct name into: AMG_int_vars separated by '~'
	from AMG_ct_cont
	where substr(name,1,3) = "AMG" 
	;
    %let intcnt = &sqlobs;
...
%put &AMG_int_vars &intcnt;



proc sql noprint ;
	select distinct name into: AMG_cond_vars separated by '~'
	from AMG_ct_cont
...
%cond_int_ph_wt(anlz_ds=AMG_ct_da, cond_mvar=&AMG_cond_vars., num_cond=&condcnt., int_mvar=&AMG_int_vars., num_int=&intcnt.);
...
%sankey_nodes(inds=all_cond_int_ph, outds=outds.AMG_sankey, nodes=%str(spnsr|cond|int|phases), cond=);
...
%sankey2html(in_data=%nrbquote(&sankeydata.), outfl=%sysfunc(pathname(outg,f))/AMG.html, width=1900, height=600, flow_num=);
